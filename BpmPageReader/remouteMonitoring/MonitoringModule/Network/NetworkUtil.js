function NetworkUtil() {
	...
	this.tabId = 0;
	this.startWatch = function(tabId, callback, onConnected) {
		var scope = this;
		var version = "1.2";
		this.tabId = tabId;
	...
	this.processNetworkActivity = function() {
		var requestCount = 0;
		var allFrames = [];
	...
	this.getRequestCallStack = function(request) {
	...
	this.getStatistic = function(frames) {
		return $linq(frames)
			.groupBy(x => x.url, x => x)
			.select((x, fileIndex) => {
...
//Image
//chrome.debugger.sendCommand({tabId:tabId}, "Page.captureScreenshot", null, function(data) {
// 	console.log(arguments);
// 	debugger;
// 	document.getElementById('canvasImg').src = "data:image/png;base64," + data.data;
// });